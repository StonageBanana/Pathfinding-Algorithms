    ...
    def run(self, source_point: Tuple[int, int], target_point: Tuple[int, int], visual: bool = False) -> Tuple[int, List[int]]:
        ...
        if visual:
            for node1 in shortest_path:
                ...
                live_fig.suptitle(f'Dijkstras Algorithm Final Path [Distance: {shortest_distance}]')
                live_ax.legend()
                live_fig.canvas.draw()
                plt.pause(0.000001)
            sleep(2)
...
if __name__ == "__main__":
    PPA = DijkstrasAlgorithm(map_path="Path-Planning-Algorithms/Maps/demo_maps/30x10_B.png")
    PPA.visualise_graph()
    PPA.operate(True)