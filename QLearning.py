...
class QLearningPathPlanner(PathPlanningAlgorithm):
    def __init__(self, map: Map = None, map_path: str = None, alpha: float = 0.1, gamma: float = 0.9, epsilon: float = 0.1, episodes: int = 1000) -> None:
        """
        Initializes the Q-learning Path Planner
        """
    ...
    def train(self, source_point: Tuple[int, int], target_point: Tuple[int, int]):
        """
        Train the Q-learning agent with an early stopping mechanism.
        """
    ...
    def run(self, source_point: Tuple[int, int], target_point: Tuple[int, int], visual: bool = False) -> List[Tuple[int, int]]:
        """
        Runs the Q-learning Path Planner and optionally visualizes the path.
        """
        ...
        if visual:
            fig, ax = plt.subplots()
            self.map.show("Q-Learning Path", fig, ax, False)
...
if __name__ == "__main__":
    planner = QLearningPathPlanner(map_path="Path-Planning-Algorithms/Maps/demo_maps/30x10_B.png", alpha=0.1, gamma=0.9, epsilon=0.1, episodes=1000)
