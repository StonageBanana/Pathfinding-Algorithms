...
class RRTAlgorithm(PathPlanningAlgorithm):
    ...
    def run(self, source_point: Tuple[int, int], target_point: Tuple[int, int], visual: bool = False, visualise_metrics: bool = False) -> Tuple[int, List[int]]:
        ...
        # Display final path distance on the visualization
        distance_text = f'Final Path Distance: {(path_distance/10):.2f}'
        live_ax.text(0.05, 0.95, distance_text, transform=live_ax.transAxes, fontsize=12, 
...
if __name__ == "__main__":
    ...
    PPA = RRTAlgorithm(map_path="Path-Planning-Algorithms/Maps/demo_maps/30x10_B.png", target_radius=0.5, delta  =0.5, exploration_bias=0.4, max_nodes=500)
