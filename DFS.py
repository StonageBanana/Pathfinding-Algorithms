"""@package DFS
@brief Contains implementation class of Depth-First Search Algorithm along with methods to display it on a map
"""
...
class DFSAlgorithm(PathPlanningAlgorithm):
    """
    @breif Initialises DFS object
    ...
    def __init__(self, map: Map = None, map_path:str = None) -> None:
        ...
        if self.map.map_type == "Fine":
            print("[ERROR] DFS algorithm can be applied to a Coarse Map only(with lesser number of nodes)")
            exit()
    ...
    def run(self, source_point: Tuple[int, int], target_point: Tuple[int, int], visual: bool = False) -> Tuple[int, List[int]]:
        ...
        @detail DFS explores as far as possible along each branch before backtracking.
                Unlike BFS/Dijkstra/A*, the path it returns is NOT guaranteed to be
                shortest in hops or in cost -- it is whatever path the traversal
                order happened to reach the target with first. That is reported
                faithfully here rather than dressed up as optimal.
        @param source_point The starting node to find shortest path
        @param target_point The target node to find shortest path
        @return path_cost (edge count of the DFS path found, NOT guaranteed minimal), path
        """
        ...
        if visual:
            ...
            self.map.show("DFS Algorithm", live_fig, live_ax, False)
        ...
        if visual:
            for node1 in shortest_path:
                ...
                live_fig.suptitle(f'DFS Algorithm Final Path [Edges: {path_cost}]')
                live_ax.legend()
                live_fig.canvas.draw()
                plt.pause(0.000001)
                sleep(0.1)
...
if __name__ == "__main__":
    PPA = DFSAlgorithm(map_path="Path-Planning-Algorithms/Maps/demo_maps/30x10_B.png")
    PPA.visualise_graph()
    PPA.operate(True)
